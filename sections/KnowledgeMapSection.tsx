...
import { ZoomIn, ZoomOut, RotateCcw, Network, Sparkles, X, ArrowRight, Layers } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <div className="text-center space-y-4">
          ...
          <h2 className="text-3xl sm:text-5xl font-bold font-serif tracking-tight text-white">
            BẢN ĐỒ TRI THỨC KINHTẾ CHÍNH TRỊ
          </h2>
          <p className="text-sm sm:text-base text-slate-400 max-w-3xl mx-auto leading-relaxed">
            Khám phá mạng lưới liên kết giữa Lý thuyết Chương 3, 5 Chức năng Tiền tệ, Sự vận động của Tư bản, Bài phân tích trung tâm và Dẫn chứng thực tiễn.
          </p>
        </div>
        ...
        {/* Interactive Mind Map Graph Board */}
        <div className="relative w-full h-[550px] rounded-3xl bg-slate-900/60 border border-slate-800 overflow-hidden shadow-2xl">
          <div
                ...
                node.connectedTo.map((targetId) => {
                  ...
                  return (
                    <line
                      ...
                      x2={`${targetNode.x}%`}
                      y2={`${targetNode.y}%`}
                      stroke={selectedNode?.id === node.id || selectedNode?.id === targetId ? "#f59e0b" : "#334155"}
            ...
            {filteredNodes.map((node) => {
              ...
              return (
                <motion.div
                  ...
                  className={`absolute -translate-x-1/2 -translate-y-1/2 p-3 sm:p-4 rounded-2xl cursor-pointer transition-all shadow-xl flex items-center space-x-2 border z-10 ${
                    isCenter
                      ? "bg-gradient-to-r from-red-600 via-amber-500 to-yellow-500 text-slate-950 font-extrabold border-amber-300 ring-4 ring-amber-500/30 scale-110"
                      : isSelected
                      ? "bg-amber-500 text-slate-950 font-bold border-amber-300 ring-2 ring-amber-400"
        ...
        <AnimatePresence>
          {selectedNode && (
            <motion.div
              ...
              exit={{ opacity: 0, y: 20 }}
              className="p-6 rounded-3xl bg-slate-900 border border-amber-500/60 shadow-2xl space-y-4 relative"
            >
              <button
              ...
              <div className="space-y-1">
                <span className="text-xs font-mono font-bold uppercase text-amber-400 tracking-wider">
                  Nút Tri trí: {selectedNode.category}
                </span>
              ...
              <p className="text-sm text-slate-300 leading-relaxed font-sans bg-slate-950 p-4 rounded-xl border border-slate-800">
                {selectedNode.description}
              </p>

              <div className="flex items-center space-x-2 text-xs text-slate-400">
                <span>Nút liên kết trực tiếp:</span>
                {selectedNode.connectedTo.map((targetId) => {
                  const target = KNOWLEDGE_MAP_NODES.find((n) => n.id === targetId);
                  return (
                    <span
                      key={targetId}
                      className="px-2 py-0.5 rounded bg-slate-800 text-amber-300 font-medium"
                    >
                      {target?.label}
                    </span>
                  );
